...
- перед билдом синхронизирует корневые `data/` и `state/` в `web/` (если есть); на Vercel используются уже закоммиченные `web/data` и `web/state`.

**Один репозиторий:** весь код и данные для прода в NewType. В настройках Vercel укажи **Root Directory** = `web`.

"""Letterform Archive news crawler. Uses WordPress REST API."""
...
class LetterformArchiveNewsCrawler:
    ...
    def crawl(self, session: requests.Session, timeout: int = 20) -> list[FontNewsItem]:
        ...
        base_url = str(
            self.source_config.get("base_url", "https://letterformarchive.org")
        ).rstrip("/")
        crawl_cfg = self.source_config.get("crawl", {})
        per_page = int(crawl_cfg.get("per_page", 100))
        max_pages = int(crawl_cfg.get("max_pages", 5))
...
def _extract_featured_image(post: dict) -> str | None:
    ...
    if not isinstance(media, dict):
        return None
    # Prefer medium_large or medium size, fall back to source_url.

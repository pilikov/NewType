"""
Watermarks for daily (incremental) crawl runs.

Stored in state/daily_watermarks.json. Each source has last_run_utc and last_date
so the next daily run can restrict to [last_date, today] without touching main parsers.
"""
...
def get_watermarks_path(state_dir: Path) -> Path:
    return state_dir / "daily_watermarks.json"


def load_daily_watermarks(state_dir: Path) -> dict[str, dict[str, Any]]:
    """Load watermarks: { source_id: { "last_run_utc": "...", "last_date": "YYYY-MM-DD" } }."""
    path = get_watermarks_path(state_dir)
...
def save_daily_watermarks(state_dir: Path, watermarks: dict[str, dict[str, Any]]) -> None:
    path = get_watermarks_path(state_dir)
...
def daily_start_end_dates(
    ...
    *,
    ...
    """
    today = date.today()
    w = get_source_watermark(watermarks, source_id)
